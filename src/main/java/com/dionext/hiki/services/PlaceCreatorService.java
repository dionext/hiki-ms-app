...
public class PlaceCreatorService extends HikingLandPageCreatorService {
    ...
    private String generatePlacePage(JGeoWikidata item, String src) {
        ...
        pageInfo.setPageTitle(item != null ? item.GetFullLabel(ru) : (i18n.getString("places.countries", pageInfo.getLocale()))); //to do add site indo
        ...
        return createHtmlAll(new SrcPageContent(body.toString(),
                i18n.getString("places.region", pageInfo.getLocale()))
        );
    }
    ...
    private String generatePlaceBlock(JGeoWikidata item, boolean singlePage) {
        if (singlePage) {
            ...
            pageInfo.setKeywords(item.GetLabel(pageInfo.getRu()) + (pageInfo.getKeywords() != null ? ", " + pageInfo.getKeywords() : ""));

            pageInfo.setDescription(i18n.getString("places.region", pageInfo.getLocale())
                    + item.GetLabel(pageInfo.getRu()) + "." + (pageInfo.getDescription() != null ? " " + pageInfo.getDescription() : ""));
            //to do image
        }
        StringBuilder str = new StringBuilder();
        if (singlePage) {
            ...
            str.append("<h1>");
            str.append(item.GetFullLabel(pageInfo.getRu()));
            str.append("</h1>");
        }
        ...
        if (!singlePage) {
            ...
            str.append("""
                    <h3 class="card-title">""");
            str.append(item.GetLabel(pageInfo.getRu()));
            str.append("""
                    </h3>""");
        } else//single page
        {
            ...
            if (item.getCoatOfArmsImage() != null) {
                ...
                String href = "https://commons.wikimedia.org/wiki/File:" + HtmlUtils.urlEncodePath(item.getCoatOfArmsImage());
...
//C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'out' keyword - these cannot be converted using the 'OutObject' helper class unless the method is within the code being modified:
        ...
        str.append(generateBlockAttributeLine(null, item.getDescription(pageInfo.getRu()))); //Ru ? "Описание" : "Description"
        if (item.getCountry() != null) {

            JGeoWikidata countryItem = jGeoWikidataRepository.findById(item.getCountry()).orElse(null);
            if (countryItem != null) {
                str.append(generateBlockAttributeLine(i18n.getString("places.country", pageInfo.getLocale()), createImage(
                        "images/countries/" + item.getJCountryId() + ".png", -1, -1,
                        countryItem.GetLabel(pageInfo.getRu()), HtmlUtils.urlEncodePath(countryItem.GetLocalUrl()), false)
                ));
        ...
        if (item.getCapital() != null) {
            ...
            if (capitalItem != null) {
                str.append(generateBlockAttributeLine(i18n.getString("places.capital", pageInfo.getLocale()), "<a href=\"" + HtmlUtils.urlEncodePath(capitalItem.GetLocalUrl()) + "\">" + capitalItem.GetLabel(pageInfo.getRu()) + "</a>"));
            }
        }
        ...
        if (singlePage) {
            ...
            if (exts != null && !exts.isEmpty()) {
                str.append("""
                        <h3><a data-toggle="collapse" href="#footwear" aria-expanded="false" aria-controls="footwear">""");
                ...
                for (var pv : propList) {
                    ...
                    str.append(generateBlockAttributeLine(pv.getProp().GetLabel(pageInfo.getRu()), valueStr));
                }
        ...
        for (var curItem1 : items) {
            if (curItem1 != null) {
                ...
                if (item != curItem1) {
                    ...
                    str.append("""
                            ">""");
                    str.append(curItem1.GetLabel(pageInfo.getRu()));
                    str.append("</a>");
                } else {
                    str.append(curItem1.GetLabel(pageInfo.getRu()));
                }
    ...
    private String makeGoogleMapHref(String lat, String lon) {
        return "https://maps.google.com/maps?ll=" + lat + "," + lon + "&q=" + lat + "," + lon + "&hl=" + pageInfo.getLang() + "&t=m&z=11";
    }
    ...
    private String makeGoogleIFrame(String lat, String lon) {
        ...
        return """
                ...
                """
                        <iframe src=\"""" +
                "https://maps.google.com/maps?ll=" + lat + "," + lon + "&q=" + lat + "," + lon + "&hl=" + pageInfo.getLang() + "&t=m&z=11&iwloc=&output=embed" +
                """
                        " frameborder="0\"""" +
                """
    ...
    private String makeWikimapiaMapHref(String lat, String lon) {
        return "http://wikimapia.org/#lang=" + pageInfo.getLang() + "&lat=" + lat + "&lon=" + lon + "&z=11&m=w";
    }

    private String makeWikilocMapHref(String lat, String lon) {
        return "https://" + pageInfo.getLang() + ".wikiloc.com/wikiloc/map.do?lt=" + lat + "&ln=" + lon + "&z=10"; //&k=1
